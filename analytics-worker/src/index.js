...
const ALLOWED_KEYS = new Set(['event_id','install_id','event','build','edition','player_count','round_count','cast_used','session_seconds','campaign','platform']);
const PLATFORMS = new Set(['android','ios','desktop','other','unknown']);
...
export function validateEvent(input) {
  ...
  return {
    ...
    campaign,platform,edition,player_count:bounded(input.player_count,2,18,'player_count'),round_count:bounded(input.round_count,0,999,'round_count'),
...
async function ingest(request,env) {
  ...
  const inserted=await env.DB.prepare(`INSERT OR IGNORE INTO events(event_id,day,install_hash,event_name,build,edition,player_count,round_count,cast_used,session_seconds,campaign,platform,is_internal) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(input.event_id,day,installHash,input.event,input.build,input.edition,input.player_count,input.round_count,input.cast_used?1:0,input.session_seconds,input.campaign,input.platform,internal).run();
  ...
  const statements=[
    env.DB.prepare(`INSERT INTO installations(install_hash,first_seen,last_seen,first_campaign,platform,is_internal) VALUES(?,datetime('now'),datetime('now'),?,?,?) ON CONFLICT(install_hash) DO UPDATE SET last_seen=datetime('now'),platform=CASE WHEN installations.platform='unknown' THEN excluded.platform ELSE installations.platform END,is_internal=MAX(installations.is_internal,excluded.is_internal)`).bind(installHash,input.campaign,input.platform,internal),
...
async function summary(request,env){
  ...
  const [installs,active7,metrics,daily,campaigns,platforms,enabled]=await Promise.all([
    ...
    env.DB.prepare(`SELECT platform,COUNT(*) AS installs FROM installations${filter} GROUP BY platform ORDER BY installs DESC`).all(),
  ...
  return json({ok:true,analytics_enabled:enabled==='true',include_internal:includeInternal,total_installations:Number(installs||0),active_7d:Number(active7||0),metrics:metric,daily:daily.results,campaigns:campaigns.results,platforms:platforms.results,generated_at:new Date().toISOString()});
}
