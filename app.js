import {MODIFIERS, calculateRound, totalsFor, gameOutcome, playerStats} from './rules.js?v=40';
...
function castPayload(game,includeDraft=false,status='active'){const totals=totalsFor(game);return {version:5,gameId:game.id,status,winnerId:status==='winner'?game.winnerId:null,target:game.target,round:game.rounds.length+1,completedRounds:game.rounds.length,roundActive:includeDraft,players:game.playerIds.map(id=>{const roundPoints=includeDraft?draftScore(id):0;const d=includeDraft&&scoringMode==='cards'?draft[id]:null;const score=totals[id]+roundPoints;return {id,name:player(id)?.name||'Player',banked:totals[id],roundPoints,score,flip7:Boolean(d?.flip7),frozen:Boolean(d?.frozen),busted:Boolean(d?.busted),doubled:Boolean(d?.doubled),hot:roundPoints>60,nearVictory:score>0&&score>=game.target-25}})}}
...
function scoreCard(id){const d=draft[id];const calc=calculateRound(d);const invalid=scoreErrors.has(id)?' score-card-error':'';const game=activeGame();const banked=game?totalsFor(game)[id]||0:0;if(scoringMode==='quick'){const points=d.quick===''?0:Number(d.quick);const projected=banked+points;const hot=d.quick!==''&&points>60;const near=d.quick!==''&&game&&projected>0&&projected>=game.target-25;const badges=`${hot?'<span class="effect-badge fire">🔥 HOT ROUND</span>':''}${near?'<span class="effect-badge electric">⚡ NEAR WIN</span>':''}`;return `<section class="card score-card${hot?' effect-hot':''}${near?' effect-near':''}${invalid}" data-player="${id}"><div class="score-card-head"><div><h3>${esc(player(id)?.name)}</h3><span class="score-effects">${badges}</span></div><span class="score-value">${d.quick===''?'—':points}</span></div><input class="quick-score" type="number" min="0" max="999" inputmode="numeric" value="${esc(d.quick)}" placeholder="Round score"><p class="score-required">Enter a score for this player</p></section>`}const projected=banked+calc;const near=Boolean(game&&projected>0&&projected>=game.target-25);const effects=`${d.flip7?' effect-flip7':''}${d.frozen?' effect-frozen':''}${calc>60?' effect-hot':''}${near?' effect-near':''}${d.doublePulse?' effect-double-pulse':''}`;const badges=`${d.flip7?'<span class="effect-badge gold">✦ FLIP 7</span>':''}${d.frozen?'<span class="effect-badge ice">❄ FROZEN</span>':''}${d.doubled?'<span class="effect-badge double">×2</span>':''}${calc>60?'<span class="effect-badge fire">🔥 HOT ROUND</span>':''}${near?'<span class="effect-badge electric">⚡ NEAR WIN</span>':''}`;return `<section class="card score-card ${d.expanded?'expanded':'collapsed'}${effects}${invalid}" data-player="${id}"><div class="score-card-head"><div><h3>${esc(player(id)?.name)}</h3><span class="score-effects">${badges}</span></div><span class="score-value">${calc}</span></div><div class="compact-score-actions"><button class="chip bust ${d.busted?'active':''}">Bust = 0</button><button class="button ghost small expand-calculator" aria-expanded="${d.expanded}">${d.expanded?'Minimize':'Enter cards'}</button></div><p class="score-required">Enter cards, Frozen, or select Bust</p>${d.expanded?`<div class="calculator-controls"><div class="number-grid">${Array.from({length:13},(_,n)=>`<button class="chip number ${d.numbers.includes(n)?'active':''}" data-number="${n}">${n}</button>`).join('')}</div><div class="specials"><button class="chip double ${d.doubled?'active':''}">×2</button>${MODIFIERS.map(n=>`<button class="chip modifier ${d.modifiers.includes(n)?'active':''}" data-mod="${n}">+${n}</button>`).join('')}<button class="chip flip ${d.flip7?'active':''}">Flip 7 +15</button><button class="chip frozen ${d.frozen?'active':''}">❄ Frozen</button></div></div>`:''}</section>`}
