import {MODIFIERS, calculateRound, totalsFor, gameOutcome, playerStats} from './rules.js?v=63';
...
function home(){const game=activeGame();return `<svg class="cast-arrow-overlay" aria-hidden="true"><path pathLength="1"/><polyline/></svg><section class="hero"><h1>Keep score.<br><span>Press your luck.</span></h1><div class="hero-cast-hint">Cast your scoreboard</div><p>Fast round scoring, offline game history, and all-time bragging rights.</p></section><section class="grid two">
  ...
  <button class="card home-action" data-nav="history"><strong>Game history</strong><span>${state.games.filter(g=>g.status==='complete').length} completed games</span></button></section><p class="subtle app-footer"><a href="${esc(feedbackUrl())}" target="_blank" rel="noopener">Feedback</a><span aria-hidden="true">·</span><a href="privacy.html?v=63">Privacy</a></p>`}
...
function scoreFx({flip7=false,frozen=false,hot=false,near=false}={}){return `<span class="score-card-fx" aria-hidden="true">${hot?'<span class="fx-sprite fx-fire"></span>':''}${frozen?'<span class="fx-sprite fx-frost-art"></span>':''}${near?'<span class="fx-near-edge"><i></i><i></i><b></b><b></b></span>':''}${flip7?`<span class="fx-flip-cards">${Array.from({length:7},(_,i)=>`<i style="--i:${i}"></i>`).join('')}</span>`:''}</span>`}
...
if('serviceWorker'in navigator){
  ...
  navigator.serviceWorker.register('./sw.js?v=63',{updateViaCache:'none'}).then(reg=>reg.update()).catch(()=>{});
}
