import {MODIFIERS, calculateRound, totalsFor, gameOutcome, playerStats} from './rules.js?v=27';
...
function castPayload(game,includeDraft=false,status='active'){const totals=totalsFor(game);return {version:3,gameId:game.id,status,winnerId:status==='winner'?game.winnerId:null,target:game.target,round:game.rounds.length+1,completedRounds:game.rounds.length,roundActive:includeDraft,players:game.playerIds.map(id=>{const roundPoints=includeDraft?draftScore(id):0;return {id,name:player(id)?.name||'Player',banked:totals[id],roundPoints,score:totals[id]+roundPoints}})}}
...
function render(){
  ...
  bind(); app.focus({preventScroll:true});if(view==='home')requestAnimationFrame(positionCastArrow);
}
function home(){const game=activeGame();return `<svg class="cast-arrow-overlay" aria-hidden="true"><path pathLength="1"/><polyline/></svg><section class="hero"><h1>Keep score.<br><span>Press your luck.</span></h1><div class="hero-cast-hint">Cast your scoreboard</div><p>Fast round scoring, offline game history, and all-time bragging rights.</p></section><section class="grid two">
  ...
  <button class="card home-action" data-nav="history"><strong>Game history</strong><span>${state.games.filter(g=>g.status==='complete').length} completed games</span></button></section><p class="subtle app-footer">Version 27 · <a href="privacy.html">Privacy</a></p>`}
...
function setup(){return `<div class="section-head setup-head"><h1>New game</h1><button class="button ghost small" data-nav="home">Cancel</button></div><section class="card setup-card">
  ...
  <div class="setup-start-bar"><button id="startGame" class="button full">Start game</button></div></section>`}
function gameScreen(game){const totals=totalsFor(game); const order=[...game.playerIds].sort((a,b)=>totals[b]-totals[a]);return `<div class="section-head"><div><h1>Scoreboard</h1><span class="subtle">First to ${game.target}</span></div><div><button class="button ghost small" data-nav="home">Home</button></div></div>
  <div class="round-banner">${game.rounds.length ? `${game.rounds.length} round${game.rounds.length===1?'':'s'} completed` : 'Ready for round 1'}</div><section class="scoreboard">${order.map((id,i)=>`<div class="score-row"><span class="rank">#${i+1}</span><strong>${esc(player(id)?.name)}</strong><span class="total">${totals[id]}</span><div class="progress"><span style="width:${Math.min(100,totals[id]/game.target*100)}%"></span></div></div>`).join('')}</section>
  ${game.rounds.length?`<section class="card" style="margin-top:14px"><div class="section-head"><h2>Rounds</h2><button id="undoRound" class="button ghost small">Undo last</button></div>${game.rounds.slice().reverse().map((r,idx)=>`<div class="history-row"><div><strong>Round ${game.rounds.length-idx}</strong><p>${game.playerIds.map(id=>`${esc(player(id)?.name)} ${r.scores[id]}`).join(' · ')}</p></div></div>`).join('')}</section>`:''}
...
function initDraft(game){draft={};for(const id of game.playerIds)draft[id]={quick:'',numbers:[],doubled:false,modifiers:[],flip7:false,busted:false,expanded:false}}
...
function scoreCard(id){const d=draft[id];const calc=calculateRound(d);const invalid=scoreErrors.has(id)?' score-card-error':'';if(scoringMode==='quick')return `<section class="card score-card${invalid}" data-player="${id}"><div class="score-card-head"><h3>${esc(player(id)?.name)}</h3><span class="score-value">${d.quick===''?'—':Number(d.quick)}</span></div><input class="quick-score" type="number" min="0" max="999" inputmode="numeric" value="${esc(d.quick)}" placeholder="Round score"><p class="score-required">Enter a score for this player</p></section>`;return `<section class="card score-card ${d.expanded?'expanded':'collapsed'}${invalid}" data-player="${id}"><div class="score-card-head"><h3>${esc(player(id)?.name)}</h3><span class="score-value">${calc}</span></div><div class="compact-score-actions"><button class="chip bust ${d.busted?'active':''}">Bust = 0</button><button class="button ghost small expand-calculator" aria-expanded="${d.expanded}">${d.expanded?'Minimize':'Enter cards'}</button></div><p class="score-required">Enter cards or select Bust</p>${d.expanded?`<div class="calculator-controls"><div class="number-grid">${Array.from({length:13},(_,n)=>`<button class="chip number ${d.numbers.includes(n)?'active':''}" data-number="${n}">${n}</button>`).join('')}</div><div class="specials"><button class="chip double ${d.doubled?'active':''}">×2</button>${MODIFIERS.map(n=>`<button class="chip modifier ${d.modifiers.includes(n)?'active':''}" data-mod="${n}">+${n}</button>`).join('')}<button class="chip flip ${d.flip7?'active':''}">Flip 7 +15</button></div></div>`:''}</section>`}
...
function hasCardScore(d){return d.busted||d.numbers.length>0||d.modifiers.length>0||d.doubled||d.flip7}
...
let deferredInstall;window.addEventListener('beforeinstallprompt',e=>{e.preventDefault();deferredInstall=e;document.querySelector('#installBtn').classList.remove('hidden')});
document.querySelector('#installBtn').onclick=async()=>{if(deferredInstall){deferredInstall.prompt();await deferredInstall.userChoice;deferredInstall=null;document.querySelector('#installBtn').classList.add('hidden')}};
...
window.addEventListener('sevenup-cast-connected',()=>sendToCast());
...
window.addEventListener('popstate',event=>{
  if(event.state?.sevenUpView){
    ...
    scrollTo(0,0);if(view==='game')sendToCast(activeGame(),false);
  }
});
