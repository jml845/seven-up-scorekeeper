(function(){
  ...
  const BUILD=90,ID_KEY='flipcast-anonymous-install-v1',OPT_OUT_KEY='flipcast-analytics-opt-out-v1',QUEUE_KEY='flipcast-analytics-queue-v1',CAMPAIGN_KEY='flipcast-analytics-campaign-v1',OPEN_KEY='flipcast-analytics-last-open-v1',CAST_GAME_KEY='flipcast-analytics-cast-game-v1';
  ...
  function platform(){const p=navigator.userAgentData?.platform||navigator.platform||'';if(/Android/i.test(p))return'android';if(/iPhone|iPad|iPod|iOS/i.test(p)||(p==='MacIntel'&&navigator.maxTouchPoints>1))return'ios';if(navigator.userAgentData?.mobile)return'other';return'desktop'}
  ...
  function track(event,detail={}){if(!enabled()||!EVENT_FIELDS[event])return;const payload={event_id:uuid(),install_id:installId(),event,build:BUILD,campaign:campaign(),platform:platform()};for(const key of EVENT_FIELDS[event])if(detail[key]!==undefined)payload[key]=detail[key];deliver(payload).catch(()=>{const pending=queue();pending.push(payload);saveQueue(pending)})}
