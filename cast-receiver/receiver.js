(function(){
  ...
  const vfxVideo=(name,key)=>{const asset=VFX[name];return `<video class="fx-video fx-${name}-video" data-effect-key="${esc(key)}"${asset.holdAt?` data-hold-at="${asset.holdAt}"`:''} autoplay muted playsinline preload="auto"><source src="assets/${asset.file}?v=77" type="${asset.type}"></video>`};
  ...
  const frozenFx=(playerId,effect)=>{if(!effect)return '';const asset=VFX[effect];return `<span class="player-fx effect-frozen fx-${effect}-frozen${effect==='x2'?' x2-frozen':''}" data-held-effect-key="${esc(`${playerId}:${effect}`)}" aria-hidden="true"><img class="fx-poster fx-${effect}-poster" src="assets/${asset.poster}?v=77" alt="">${effect==='x2'?'<span>×2</span>':''}</span>`};
  function warmVfx(){for(const asset of Object.values(VFX)){const image=new Image();image.decoding='async';image.src=`assets/${asset.poster}?v=77`}}
  ...
  function revealVideo(video){if(!video?.isConnected||video.classList.contains('ready'))return;clearTimeout(video._startTimer);clearTimeout(video._frameTimer);video._startTimer=null;video.classList.add('ready');video.closest('.player-fx')?.classList.add('video-ready')}
  ...
  board.addEventListener('timeupdate',event=>{const video=event.target;if(!video.matches('.fx-video'))return;if(!video.classList.contains('ready')&&video.currentTime>0)revealVideo(video);if(!video.matches('[data-hold-at]'))return;const holdAt=Number(video.dataset.holdAt);if(video.currentTime>=holdAt)finishVideo(video)},true);
  board.addEventListener('error',event=>{if(event.target.matches('.fx-video'))playbackFailed(event.target)},true);
  board.addEventListener('playing',event=>{const video=event.target;if(!video.matches('.fx-video')||video.classList.contains('ready'))return;clearTimeout(video._stallTimer);if(video.requestVideoFrameCallback)video._frameRequest=video.requestVideoFrameCallback(()=>revealVideo(video));else video._frameTimer=setTimeout(()=>{if(video.currentTime>0)revealVideo(video)},80)},true);
