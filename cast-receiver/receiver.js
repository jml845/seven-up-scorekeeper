(function(){
  ...
  const posterFile=name=>name==='bust'||name==='flip7'?`${name}-v58-poster.png`:`${name}-v53-poster.png`;
  const poster=(name,active)=>`<img class="fx-poster fx-${name}-poster${active?' active-poster':''}" src="assets/${posterFile(name)}?v=58" alt="">`;
  const vfxVideo=(name,key)=>{const asset=VFX[name];return `<video class="fx-video fx-${name}-video" data-effect-key="${esc(key)}"${asset.holdAt?` data-hold-at="${asset.holdAt}"`:''} autoplay muted playsinline preload="auto" poster="assets/${posterFile(name)}?v=58" onloadeddata="this.classList.add('ready');this.closest('.player-fx').classList.add('video-ready')" onplaying="this.classList.add('ready');this.closest('.player-fx').classList.add('video-ready')"><source src="assets/${asset.file}?v=58" type="${asset.type}"></video>`};
  ...
  function warmVfx(){const bin=document.createElement('div');bin.className='vfx-preload';for(const [name,asset] of Object.entries(VFX)){const video=document.createElement('video');video.muted=true;video.playsInline=true;video.preload='auto';video.poster=`assets/${posterFile(name)}?v=58`;video.src=`assets/${asset.file}?v=58`;video.load();bin.appendChild(video)}document.body.appendChild(bin)}
  let lastGameId=null,previousLeaderId=null,previousDoubled=new Map(),effectState=new Map();
  function finishVideo(video,failed=false){const key=video.dataset.effectKey;if(key)effectState.set(key,'done');const wrap=video.closest('.player-fx');if(failed)video.classList.add('failed');video.remove();if(wrap){wrap.classList.remove('video-ready');if(failed)wrap.classList.add('video-failed')}}
