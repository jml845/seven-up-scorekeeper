(function(){
  ...
  let lastGameId=null,lastRound=null,previousLeaderId=null,previousDoubled=new Map(),effectState=new Map(),playbackRetries=new Map(),effectQueue=[],activeEffectKeys=new Set(),lastData=null,decoderLimit=MAX_ACTIVE_EFFECTS;
  ...
  function clearEffects(){discardVideo();effectQueue=[];activeEffectKeys.clear();effectState=new Map();playbackRetries.clear();decoderLimit=MAX_ACTIVE_EFFECTS}
  ...
  function render(data){
    ...
    for(const key of effectState.keys())if(!activeKeys.has(key)){effectState.delete(key);playbackRetries.delete(key);effectQueue=effectQueue.filter(item=>item!==key);activeEffectKeys.delete(key);discardVideo(key)}
