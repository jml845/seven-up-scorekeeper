(function(){
  ...
  const vfxVideo=(name,key)=>{const asset=VFX[name];return `<video class="fx-video fx-${name}-video" data-effect-key="${esc(key)}"${asset.holdAt?` data-hold-at="${asset.holdAt}"`:''} autoplay muted playsinline preload="auto"><source src="assets/${asset.file}?v=73" type="${asset.type}"></video>`};
  ...
  const MAX_ACTIVE_EFFECTS=1,START_TIMEOUT_MS=4500,STALL_GRACE_MS=1400,MAX_PLAYBACK_RETRIES=1;
  const fx=(p,activeEffect)=>{const key=activeEffect?`${p.id}:${activeEffect}`:'';const asset=activeEffect&&VFX[activeEffect];return `<span class="player-fx${activeEffect?` has-effect has-video fx-stage-${activeEffect}`:''}" aria-hidden="true">${activeEffect?`<img class="fx-poster fx-${activeEffect}-poster active-poster" src="assets/${asset.poster}?v=73" alt="">${vfxVideo(activeEffect,key)}`:''}${activeEffect==='x2'?'<span class="fx-x2-hold">×2</span>':''}</span>`};
  function warmVfx(){for(const asset of Object.values(VFX)){const image=new Image();image.decoding='async';image.src=`assets/${asset.poster}?v=73`}}
  let lastGameId=null,lastRound=null,previousLeaderId=null,previousDoubled=new Map(),effectState=new Map(),playbackRetries=new Map(),effectQueue=[],activeEffectKeys=new Set(),x2HeldPlayers=new Set(),lastData=null,decoderLimit=MAX_ACTIVE_EFFECTS;
  ...
  function clearEffects(){discardVideo();effectQueue=[];activeEffectKeys.clear();x2HeldPlayers.clear();effectState=new Map();playbackRetries.clear();decoderLimit=MAX_ACTIVE_EFFECTS}
  ...
  function finishVideo(video){if(!video||video.dataset.finished==='1')return;video.dataset.finished='1';clearTimeout(video._startTimer);clearTimeout(video._stallTimer);video.pause();const key=video.dataset.effectKey;if(key){activeEffectKeys.delete(key);effectState.set(key,'done');if(key.endsWith(':x2'))x2HeldPlayers.add(playerKey(key))}video.remove();setTimeout(()=>lastData&&render(lastData),0)}
  ...
  board.addEventListener('timeupdate',event=>{const video=event.target;if(!video.matches('.fx-video[data-hold-at]'))return;const holdAt=Number(video.dataset.holdAt);if(video.currentTime>=holdAt)finishVideo(video)},true);
  board.addEventListener('error',event=>{if(event.target.matches('.fx-video'))playbackFailed(event.target)},true);
  board.addEventListener('playing',event=>{const video=event.target;if(!video.matches('.fx-video'))return;clearTimeout(video._startTimer);clearTimeout(video._stallTimer);video._startTimer=null;video.classList.add('ready');video.closest('.player-fx')?.classList.add('video-ready')},true);
  ...
  function render(data){
    ...
    for(const key of effectState.keys())if(!activeKeys.has(key)){effectState.delete(key);playbackRetries.delete(key);effectQueue=effectQueue.filter(item=>item!==key);activeEffectKeys.delete(key);if(key.endsWith(':x2'))x2HeldPlayers.delete(playerKey(key));discardVideo(key)}
    ...
    board.innerHTML=`<header><div><span>${data.roundActive?'CURRENT ROUND PREVIEW':'LIVE SCOREBOARD'}</span><h1>FlipCast</h1></div><div class="round">Round <b>${Number(data.round)||1}</b><small>First to ${Number(data.target)||200}</small></div></header><div class="players">${players.map((p,i)=>{const doublePulse=p.doubled&&previousDoubled.get(p.id)===false;const effects=`${p.flip7?' flip7':''}${p.frozen?' frozen':''}${p.hot?' hot':''}${p.busted?' busted':''}${p.nearVictory?' near-win':''}${doublePulse?' double-pulse':''}${p.id===newLeaderId?' new-leader':''}`;const badges=`${p.flip7?'<small class="effect-tag gold">✦ FLIP 7</small>':''}${p.frozen?'<small class="effect-tag ice">❄ FROZEN</small>':''}${p.doubled?'<small class="effect-tag double">×2</small>':''}${p.hot?'<small class="effect-tag fire">🔥 60+</small>':''}${p.nearVictory?'<small class="effect-tag electric">⚡ NEAR WIN</small>':''}${p.busted?'<small class="effect-tag bust">BUST</small>':''}`;const active=activeEffectById.get(p.id),heldX2=x2HeldPlayers.has(String(p.id))?'<span class="player-fx x2-frozen"><img class="fx-poster fx-x2-poster" src="assets/x2-v53-poster.png?v=74" alt=""><span>×2</span></span>':'';return `<article class="player ${p.score===high&&high>0?'leader':''} ${data.roundActive?'preview':''}${effects}">${heldX2}${fx(p,active?.effect)}<em>${i===0?'♛':i+1}</em><strong>${esc(p.name)}<span class="effect-tags">${badges}</span>${p.score===high&&high>0?'<small class="leader-label">LEADER</small>':''}</strong><i><u style="width:${Math.min(100,p.score/data.target*100)}%"></u></i><b>${Number(p.score)||0}${data.roundActive?`<small><span>${Number(p.banked)||0}</span> <mark>+${Number(p.roundPoints)||0}</mark></small>`:''}</b></article>`}).join('')}</div><footer>${data.roundActive?'Projected totals if everyone stops now':`${Number(data.completedRounds)||0} rounds complete · ${players.length} players`}</footer>`;
