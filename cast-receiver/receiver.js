(function(){
  ...
  function warmVfx(){const bin=document.createElement('div');bin.className='vfx-preload';for(const [name,asset] of Object.entries(VFX)){const video=document.createElement('video');video.muted=true;video.playsInline=true;video.preload='metadata';video.poster=`assets/${posterFile(name)}?v=64`;video.src=`assets/${asset.file}?v=64`;bin.appendChild(video)}document.body.appendChild(bin)}
