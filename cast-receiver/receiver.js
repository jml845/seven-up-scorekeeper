(function(){
  ...
  const CANVAS_EFFECTS=new Set(['bust','flip7','x2']);
  const vfxVideo=(name,key)=>{const asset=VFX[name];return `${CANVAS_EFFECTS.has(name)?`<canvas class="fx-canvas fx-${name}-canvas"></canvas>`:''}<video class="fx-video fx-${name}-video${CANVAS_EFFECTS.has(name)?' canvas-source':''}" data-effect-key="${esc(key)}"${asset.holdAt?` data-hold-at="${asset.holdAt}"`:''} autoplay muted playsinline preload="auto"><source src="assets/${asset.file}?v=80" type="${asset.type}"></video>`};
  ...
  const fx=(p,activeEffect)=>{const key=activeEffect?`${p.id}:${activeEffect}`:'',asset=activeEffect&&VFX[activeEffect];return `<span class="player-fx${activeEffect?` has-effect has-video fx-stage-${activeEffect}`:''}" aria-hidden="true">${activeEffect?`<img class="fx-poster fx-${activeEffect}-poster active-poster" src="assets/${asset.poster}?v=80" alt="">${vfxVideo(activeEffect,key)}`:''}</span>`};
  const frozenFx=(playerId,effect)=>{if(!effect)return '';const asset=VFX[effect];return `<span class="player-fx effect-frozen fx-${effect}-frozen${effect==='x2'?' x2-frozen':''}" data-held-effect-key="${esc(`${playerId}:${effect}`)}" aria-hidden="true"><img class="fx-poster fx-${effect}-poster" src="assets/${asset.poster}?v=80" alt="">${effect==='x2'?'<span>×2</span>':''}</span>`};
  function warmVfx(){for(const asset of Object.values(VFX)){const image=new Image();image.decoding='async';image.src=`assets/${asset.poster}?v=80`}}
