(function(){
  ...
  const frozenFx=(playerId,effect)=>{if(!effect)return '';const asset=VFX[effect];return `<span class="player-fx effect-frozen fx-${effect}-frozen${effect==='x2'?' x2-frozen':''}" data-held-effect-key="${esc(`${playerId}:${effect}`)}" aria-hidden="true"><img class="fx-poster fx-${effect}-poster" src="assets/${asset.poster}?v=95" alt="">${effect==='x2'?'<span>×2</span>':''}</span>`};
  const warmedPosters=new Set();
  function warmPoster(name){const asset=VFX[name];if(!asset||warmedPosters.has(name))return;warmedPosters.add(name);const image=new Image();image.decoding='async';image.src=`assets/${asset.poster}?v=95`}
  ...
  function mountSharedVideo(slot){if(!slot||decoderState!=='ready')return;const key=slot.dataset.effectKey,name=slot.dataset.effectName,asset=VFX[name];if(!asset)return;if(sharedVideo.dataset.effectKey!==key){resetVideoRuntime(sharedVideo);sharedVideo.pause();decoderDock.appendChild(sharedVideo);sharedVideo.className=`fx-video fx-${name}-video`;sharedVideo.dataset.effectKey=key;if(asset.holdAt)sharedVideo.dataset.holdAt=asset.holdAt;else sharedVideo.removeAttribute('data-hold-at');sharedVideo.src=`assets/${asset.file}?v=95`;sharedVideo.load()}if(sharedVideo.classList.contains('ready'))slot.replaceWith(sharedVideo);ensurePlayback(sharedVideo)}
  function revealVideo(video){if(!video?.isConnected||video.classList.contains('ready')||video.currentTime<REVEAL_AFTER_SECONDS)return false;const slot=[...board.querySelectorAll('.fx-video-slot')].find(candidate=>candidate.dataset.effectKey===video.dataset.effectKey);if(!slot)return false;clearTimeout(video._startTimer);clearTimeout(video._frameTimer);video._startTimer=null;slot.replaceWith(video);video.classList.add('ready');video.closest('.player-fx')?.classList.add('video-ready');return true}
  ...
  sharedVideo.addEventListener('ended',()=>{if(sharedVideo.matches('.fx-video'))finishVideo(sharedVideo)});
  sharedVideo.addEventListener('timeupdate',()=>{if(!sharedVideo.matches('.fx-video'))return;if(!sharedVideo.classList.contains('ready')&&!sharedVideo.requestVideoFrameCallback)revealVideo(sharedVideo);if(!sharedVideo.matches('[data-hold-at]'))return;const holdAt=Number(sharedVideo.dataset.holdAt);if(sharedVideo.currentTime>=holdAt)finishVideo(sharedVideo)});
  sharedVideo.addEventListener('error',()=>{if(sharedVideo.matches('.fx-video'))playbackFailed(sharedVideo)});
  sharedVideo.addEventListener('playing',()=>{const video=sharedVideo;if(!video.matches('.fx-video'))return;clearTimeout(video._stallTimer);if(video.classList.contains('ready'))return;clearTimeout(video._frameTimer);if(video.requestVideoFrameCallback)video._frameRequest=video.requestVideoFrameCallback((now,metadata)=>waitForVisibleFrame(video,metadata));else video._frameTimer=setTimeout(()=>revealVideo(video),220)});
  ...
  sharedVideo.addEventListener('waiting',stall);sharedVideo.addEventListener('stalled',stall);
  let wakeLock=null;
  ...
  function render(data){
    ...
    if(board.contains(sharedVideo))decoderDock.appendChild(sharedVideo);
