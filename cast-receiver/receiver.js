(function(){
  ...
  const RECEIVER_BUILD=91;
  const context=cast.framework.CastReceiverContext.getInstance();
  const idle=document.querySelector('#idle'),board=document.querySelector('#scoreboard');
  const esc=v=>String(v??'').replace(/[&<>'"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]));
  const VFX={fire:{file:'fire-v49.mp4',type:'video/mp4',poster:'fire-v53-poster.png'},freeze:{file:'freeze-v49.mp4',type:'video/mp4',poster:'freeze-v53-poster.png'},electric:{file:'electric-v85.mp4',type:'video/mp4',poster:'electric-v85-poster.png'},x2:{file:'x2-v83.mp4',type:'video/mp4',poster:'x2-v53-poster.png'},divide:{file:'divide-v91.mp4',type:'video/mp4',poster:'divide-v91-poster.png',holdAt:5.75},lucky13:{file:'lucky13-v86.mp4',type:'video/mp4',poster:'lucky13-v86-poster.png'},bust:{file:'bust-v83.mp4',type:'video/mp4',poster:'bust-v58-poster.png',holdAt:8.6},flip7:{file:'flip7-v83.mp4',type:'video/mp4',poster:'flip7-v60-poster.png'}};
  const vfxVideo=(name,key)=>{const asset=VFX[name];return `<video class="fx-video fx-${name}-video" data-effect-key="${esc(key)}"${asset.holdAt?` data-hold-at="${asset.holdAt}"`:''} muted playsinline preload="auto"><source src="assets/${asset.file}?v=91" type="${asset.type}"></video>`};
  ...
  const frozenFx=(playerId,effect)=>{if(!effect)return '';const asset=VFX[effect];return `<span class="player-fx effect-frozen fx-${effect}-frozen${effect==='x2'?' x2-frozen':''}" data-held-effect-key="${esc(`${playerId}:${effect}`)}" aria-hidden="true"><img class="fx-poster fx-${effect}-poster" src="assets/${asset.poster}?v=91" alt="">${effect==='x2'?'<span>×2</span>':''}</span>`};
  const warmedPosters=new Set();
  function warmPoster(name){const asset=VFX[name];if(!asset||warmedPosters.has(name))return;warmedPosters.add(name);const image=new Image();image.decoding='async';image.src=`assets/${asset.poster}?v=91`}
