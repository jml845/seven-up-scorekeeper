(function(){
  ...
  const poster=(name,active)=>`<img class="fx-poster fx-${name}-poster${active?' active-poster':''}" src="assets/${name}-v53-poster.png?v=54" alt="">`;
  const vfxVideo=(name,key)=>{const asset=VFX[name];return `<video class="fx-video fx-${name}-video" data-effect-key="${esc(key)}" autoplay muted playsinline preload="auto" poster="assets/${name}-v53-poster.png?v=54" onloadeddata="this.classList.add('ready');this.closest('.player-fx').classList.add('video-ready')" onplaying="this.classList.add('ready');this.closest('.player-fx').classList.add('video-ready')"><source src="assets/${asset.file}?v=54" type="${asset.type}"></video>`};
