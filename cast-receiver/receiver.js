(function(){
  ...
  const vfxVideo=(name,key)=>{const asset=VFX[name];return `<video class="fx-video fx-${name}-video" data-effect-key="${esc(key)}"${asset.holdAt?` data-hold-at="${asset.holdAt}"`:''} muted playsinline preload="auto"><source src="assets/${asset.file}?v=97" type="${asset.type}"></video>`};
  ...
  const frozenFx=(playerId,effect)=>{if(!effect)return '';const asset=VFX[effect];return `<span class="player-fx effect-frozen fx-${effect}-frozen${effect==='x2'?' x2-frozen':''}" data-held-effect-key="${esc(`${playerId}:${effect}`)}" aria-hidden="true"><img class="fx-poster fx-${effect}-poster" src="assets/${asset.poster}?v=97" alt="">${effect==='x2'?'<span>×2</span>':''}</span>`};
  const warmedPosters=new Set();
  function warmPoster(name){const asset=VFX[name];if(!asset||warmedPosters.has(name))return;warmedPosters.add(name);const image=new Image();image.decoding='async';image.src=`assets/${asset.poster}?v=97`}
  let lastGameId=null,lastRound=null,previousLeaderId=null,previousDoubled=new Map(),effectState=new Map(),playbackRetries=new Map(),effectQueue=[],activeEffectKeys=new Set(),heldEffects=new Map(),lastData=null,decoderLimit=MAX_ACTIVE_EFFECTS,primerState='waiting-game',primerFrames=0,primerTimer=null,primerVideo=null,primerGameId=null,primerAttempt=0;
  ...
  function fillEffectSlots(){
    if(primerState==='armed'&&effectQueue.length){startPrimer(4);return}
  ...
  function render(data){
    ...
    if(data.gameId!==lastGameId){lastGameId=data.gameId;lastRound=Number(data.round)||1;previousLeaderId=null;previousDoubled=new Map();clearEffects();armPrimer(data.gameId)}
  ...
  function armPrimer(gameId){removePrimer();primerGameId=String(gameId??'');primerState='armed';primerFrames=0;primerAttempt=0;publishPrimer()}
