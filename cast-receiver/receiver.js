(function(){
  ...
  const poster=(name,active)=>`<img class="fx-poster fx-${name}-poster${active?' active-poster':''}" src="assets/${posterFile(name)}?v=63" alt="">`;
  const vfxVideo=(name,key)=>{const asset=VFX[name];return `<video class="fx-video fx-${name}-video" data-effect-key="${esc(key)}"${asset.holdAt?` data-hold-at="${asset.holdAt}"`:''} autoplay muted playsinline preload="auto" onloadeddata="this.classList.add('ready');this.closest('.player-fx').classList.add('video-ready')" onplaying="this.classList.add('ready');this.closest('.player-fx').classList.add('video-ready')"><source src="assets/${asset.file}?v=63" type="${asset.type}"></video>`};
  const videoEffect=p=>p.busted?'bust':p.flip7?'flip7':p.doubled?'x2':p.frozen?'freeze':p.hot?'fire':null;
  const videoPriority=['bust','flip7','x2','freeze','fire'];
  const nearEdge='<span class="fx-near-edge"><i></i><i></i><b></b><b></b></span>';
  const fx=(p,activeVideo)=>{const names=[];if(p.hot)names.push('fire');if(p.frozen)names.push('freeze');if(p.doubled)names.push('x2');if(p.busted)names.push('bust');if(p.flip7)names.push('flip7');const key=activeVideo?`${p.id}:${activeVideo}`:'';return `<span class="player-fx${activeVideo?' has-video':''}" aria-hidden="true">${names.map(name=>poster(name,name===activeVideo)).join('')}${p.nearVictory?nearEdge:''}${p.doubled?'<span class="fx-x2-hold">×2</span>':''}${activeVideo?vfxVideo(activeVideo,key):''}</span>`};
  function warmVfx(){const bin=document.createElement('div');bin.className='vfx-preload';for(const [name,asset] of Object.entries(VFX)){const video=document.createElement('video');video.muted=true;video.playsInline=true;video.preload='metadata';video.poster=`assets/${posterFile(name)}?v=63`;video.src=`assets/${asset.file}?v=63`;bin.appendChild(video)}document.body.appendChild(bin)}
