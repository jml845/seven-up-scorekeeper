(function(){
  ...
  const vfxVideo=(name,key)=>{const asset=VFX[name];return `<video class="fx-video fx-${name}-video" data-effect-key="${esc(key)}"${asset.holdAt?` data-hold-at="${asset.holdAt}"`:''} muted playsinline preload="auto"><source src="assets/${asset.file}?v=92" type="${asset.type}"></video>`};
  ...
  const frozenFx=(playerId,effect)=>{if(!effect)return '';const asset=VFX[effect];return `<span class="player-fx effect-frozen fx-${effect}-frozen${effect==='x2'?' x2-frozen':''}" data-held-effect-key="${esc(`${playerId}:${effect}`)}" aria-hidden="true"><img class="fx-poster fx-${effect}-poster" src="assets/${asset.poster}?v=92" alt="">${effect==='x2'?'<span>×2</span>':''}</span>`};
  const warmedPosters=new Set();
  function warmPoster(name){const asset=VFX[name];if(!asset||warmedPosters.has(name))return;warmedPosters.add(name);const image=new Image();image.decoding='async';image.src=`assets/${asset.poster}?v=92`}
