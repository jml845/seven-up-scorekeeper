(function(){
  ...
  const vfxVideo=(name,klass,ext='mp4')=>`<video class="fx-video ${klass}" autoplay muted loop playsinline preload="auto" onerror="this.classList.add('failed');this.closest('.player-fx').classList.add('video-failed')"><source src="assets/${name}-v${ext==='mp4'?49:51}.${ext}?v=51" type="video/${ext}"></video>`;
