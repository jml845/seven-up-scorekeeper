(function(){
  ...
  const poster=(name,active)=>`<img class="fx-poster fx-${name}-poster${active?' active-poster':''}" src="assets/${posterFile(name)}?v=68" alt="">`;
  const vfxVideo=(name,key)=>{const asset=VFX[name];return `<video class="fx-video fx-${name}-video" data-effect-key="${esc(key)}"${asset.holdAt?` data-hold-at="${asset.holdAt}"`:''} autoplay muted playsinline preload="auto"><source src="assets/${asset.file}?v=68" type="${asset.type}"></video>`};
  ...
  const fx=(p,activeVideo)=>{const key=activeVideo?`${p.id}:${activeVideo}`:'';return `<span class="player-fx${activeVideo?' has-video':''}" aria-hidden="true">${activeVideo?poster(activeVideo,true):''}${p.doubled?'<span class="fx-x2-hold">×2</span>':''}${activeVideo?vfxVideo(activeVideo,key):''}</span>`};
  function warmVfx(){for(const name of Object.keys(VFX)){const image=new Image();image.decoding='async';image.src=`assets/${posterFile(name)}?v=68`}}
  ...
  function render(data){
    ...
    for(const key of effectState.keys())if(!activeKeys.has(key)){effectState.delete(key);effectQueue=effectQueue.filter(item=>item!==key);if(currentEffectKey===key){clearTimeout(effectWatchdog);effectWatchdog=null;currentEffectKey=null}}
