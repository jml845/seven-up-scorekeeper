(function(){
  ...
  const vfxVideo=(name,key)=>{const asset=VFX[name];return `<video class="fx-video fx-${name}-video" data-effect-key="${esc(key)}"${asset.holdAt?` data-hold-at="${asset.holdAt}"`:''} autoplay muted playsinline preload="auto"><source src="assets/${asset.file}?v=78" type="${asset.type}"></video>`};
  ...
  const fx=(p,activeEffect)=>{const key=activeEffect?`${p.id}:${activeEffect}`:'',asset=activeEffect&&VFX[activeEffect];return `<span class="player-fx${activeEffect?` has-effect has-video fx-stage-${activeEffect}`:''}" aria-hidden="true">${activeEffect?`<img class="fx-poster fx-${activeEffect}-poster active-poster" src="assets/${asset.poster}?v=78" alt="">${vfxVideo(activeEffect,key)}`:''}</span>`};
  const frozenFx=(playerId,effect)=>{if(!effect)return '';const asset=VFX[effect];return `<span class="player-fx effect-frozen fx-${effect}-frozen${effect==='x2'?' x2-frozen':''}" data-held-effect-key="${esc(`${playerId}:${effect}`)}" aria-hidden="true"><img class="fx-poster fx-${effect}-poster" src="assets/${asset.poster}?v=78" alt="">${effect==='x2'?'<span>×2</span>':''}</span>`};
  function warmVfx(){for(const asset of Object.values(VFX)){const image=new Image();image.decoding='async';image.src=`assets/${asset.poster}?v=78`}}
  ...
  function discardVideo(key){for(const video of board.querySelectorAll('.fx-video[data-effect-key]'))if(!key||video.dataset.effectKey===key){clearTimeout(video._startTimer);clearTimeout(video._stallTimer);clearTimeout(video._frameTimer);if(video._frameRequest&&video.cancelVideoFrameCallback)video.cancelVideoFrameCallback(video._frameRequest);video.pause();video.remove()}}
  ...
  function frameLooksDrawable(video){try{const canvas=frameLooksDrawable.canvas||(frameLooksDrawable.canvas=document.createElement('canvas'));canvas.width=8;canvas.height=4;const ctx=canvas.getContext('2d',{willReadFrequently:true});ctx.drawImage(video,0,0,8,4);const pixels=ctx.getImageData(0,0,8,4).data;let white=0,min=255,max=0;for(let i=0;i<pixels.length;i+=4){const value=(pixels[i]+pixels[i+1]+pixels[i+2])/3;min=Math.min(min,value);max=Math.max(max,value);if(value>247&&pixels[i+3]>245)white++}return white<29&&(max-min>12||max<235)}catch{return false}}
  function revealVideo(video){if(!video?.isConnected||video.classList.contains('ready')||video.currentTime<REVEAL_AFTER_SECONDS||!frameLooksDrawable(video))return false;clearTimeout(video._startTimer);clearTimeout(video._frameTimer);video._startTimer=null;video.classList.add('ready');video.closest('.player-fx')?.classList.add('video-ready');return true}
  function waitForVisibleFrame(video,metadata={}){if(!video?.isConnected||video.classList.contains('ready'))return;video._decodedFrames=Math.max(video._decodedFrames||0,Number(metadata.presentedFrames)||0);if(video._decodedFrames>=REVEAL_AFTER_FRAMES&&revealVideo(video))return;if(video.requestVideoFrameCallback)video._frameRequest=video.requestVideoFrameCallback((now,next)=>waitForVisibleFrame(video,next))}
  function finishVideo(video){if(!video||video.dataset.finished==='1')return;video.dataset.finished='1';clearTimeout(video._startTimer);clearTimeout(video._stallTimer);clearTimeout(video._frameTimer);if(video._frameRequest&&video.cancelVideoFrameCallback)video.cancelVideoFrameCallback(video._frameRequest);video.pause();const key=video.dataset.effectKey;if(key){activeEffectKeys.delete(key);effectState.set(key,'done');heldEffects.set(playerKey(key),key.slice(key.lastIndexOf(':')+1))}video.remove();setTimeout(()=>lastData&&render(lastData),0)}
  ...
  board.addEventListener('playing',event=>{const video=event.target;if(!video.matches('.fx-video')||video.classList.contains('ready'))return;clearTimeout(video._stallTimer);if(video.requestVideoFrameCallback)video._frameRequest=video.requestVideoFrameCallback((now,metadata)=>waitForVisibleFrame(video,metadata));else video._frameTimer=setTimeout(()=>revealVideo(video),160)},true);
