...
assert.match(index,new RegExp(`analytics\\.js\\?v=${build}`),'the versioned analytics client must load before app startup');
...
assert.match(sender,/sevenup-cast-disconnected/,'Cast session endings must emit an anonymous lifecycle event');
