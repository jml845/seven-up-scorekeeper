...
assert.match(sender,/decoderState = String\(message\.decoderState\)/,'Sender diagnostics must capture the receiver decoder state');
assert.match(sender,/message\.type === 'DECODER'/,'Sender diagnostics must accept asynchronous decoder-state updates');
...
assert.match(receiver,/function parkVideo\(\).*sharedVideo\.pause\(\).*decoderDock\.appendChild\(sharedVideo\)/,'Abandoned effects must park the persistent decoder surface instead of destroying it');
assert.match(receiver,/if\(decoderState!==['"]ready['"]\)return/,'The first requested effect must wait for session decoder priming');
assert.match(receiver,/startDecoderPrimer\(\)/,'The Cast receiver must prime its decoder once per session');
assert.match(receiver,/decoderState='failed'/,'A failed primer must fail closed without exposing a blank video surface');
assert.match(receiver,/publishDecoderState\(\)/,'Decoder readiness changes must be reported back to connected senders');
assert.doesNotMatch(receiverCss,/decoder-surface[^}]*visibility:hidden/,'The primer surface must remain compositable so Chromecast actually initializes its decoder');
assert.match(receiver,/const sharedVideo=document\.createElement\(['"]video['"]\)/,'All effects must reuse one persistent video element');
